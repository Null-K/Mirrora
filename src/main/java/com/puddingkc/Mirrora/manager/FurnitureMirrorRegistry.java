...
public class FurnitureMirrorRegistry {

    private final JavaPlugin plugin;
    private final File file;
    private final Map<String, FurnitureMirrorConfig> configs = new ConcurrentHashMap<>();

    public FurnitureMirrorRegistry(JavaPlugin plugin) {
        this.plugin = plugin;
        this.file = new File(plugin.getDataFolder(), "furniture-mirrors.yml");
    }
    ...
    public void load() {
        ...
        if (!file.exists()) {
            plugin.saveResource("furniture-mirrors.yml", false);
        }

        YamlConfiguration config = YamlConfiguration.loadConfiguration(file);
        ConfigurationSection root = config.getConfigurationSection("furniture-mirrors");
        if (root == null) {
            return;
        }

        for (String furnitureId : root.getKeys(false)) {
            ConfigurationSection section = root.getConfigurationSection(furnitureId);
            if (section == null) {
                continue;
            }
            ...
            configs.put(furnitureId, new FurnitureMirrorConfig(furnitureId, width, height, depth,
                    extendDown, offsetForward, offsetRight, offsetY));
        }
    }

    public FurnitureMirrorConfig get(String furnitureId) {
        return configs.get(furnitureId);
    }
}
